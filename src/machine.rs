...
impl Machine {
    pub fn new(rom: ROM, force_model: Option<GameBoyModel>) -> Self {
        ...
        Self {
            ...
            screen: screen.clone(),
            cpu: Rc::new(RefCell::new(CPU::new(model))),
    ...
    pub fn start(&mut self, skip_bootrom: bool) {
        ...
        bus.map_range_write(0x0000..=0x7FFF, closure!(clone self.rom, |addr, data| rom.borrow().write_byte(addr, data)));

        // 8000-9FFF - VRAM
        bus.map_range_read(0x8000..=0x9FFF, closure!(clone self.ppu, |addr| ppu.borrow().read_byte(addr)));
        bus.map_range_write(0x8000..=0x9FFF, closure!(clone self.ppu, |addr, data| ppu.borrow().write_byte(addr, data)));

        // A000-BFFF - ROM External RAM
        bus.map_range_read(0xA000..=0xBFFF, closure!(clone self.rom, |addr| rom.borrow().read_byte(addr)));
        bus.map_range_write(0xA000..=0xBFFF, closure!(clone self.rom, |addr, data| rom.borrow().write_byte(addr, data)));

        // C000-CFFF - WRAM Bank 0
        bus.map_range_read(0xC000..=0xCFFF, closure!(clone self.ram1, |addr| ram1.borrow().read_byte(addr)));
        bus.map_range_write(0xC000..=0xCFFF, closure!(clone self.ram1, |addr, data| ram1.borrow().write_byte(addr, data)));

        // D000-DFFF - WRAM Banks 1-7
        bus.map_range_read(0xD000..=0xDFFF, closure!(clone self.ram2, |addr| ram2.borrow().read_byte(addr)));
        bus.map_range_write(0xD000..=0xDFFF, closure!(clone self.ram2, |addr, data| ram2.borrow().write_byte(addr, data)));
        ...
        bus.map_range_write(0xE000..=0xEFFF, closure!(clone self.ram1, |addr, data| ram1.borrow().write_byte(addr - 0xE000 + 0xC000, data)));
        ...
        bus.map_range_write(0xF000..=0xFDFF, closure!(clone self.ram2, |addr, data| ram2.borrow().write_byte(addr - 0xF000 + 0xD000, data)));

        // FE00-FE9F - OAM Table
        bus.map_range_read(0xFE00..=0xFE9F, closure!(clone self.ppu, |addr| ppu.borrow().read_oam_byte(addr - 0xFE00)));
        bus.map_range_write(0xFE00..=0xFE9F, closure!(clone self.ppu, |addr, data| ppu.borrow().write_oam_byte(addr - 0xFE00, data)));

        // FEA0-FEFF Unusable

        // FF00 - Joystick Register
        bus.map_address_read(0xFF00, closure!(clone self.joystick, |addr| joystick.borrow().read_byte(addr)));
        bus.map_address_write(0xFF00, closure!(clone self.joystick, |addr, data| joystick.borrow().write_byte(addr, data)));

        // FF01-FF02 - Serial 
        bus.map_range_read(0xFF01..=0xFF02, closure!(clone self.serial, |addr| serial.borrow().read_byte(addr)));
        bus.map_range_write(0xFF01..=0xFF02, closure!(clone self.serial, |addr, data| serial.borrow().write_byte(addr, data)));

        // FF04-FF07 - Timer
        bus.map_range_read(0xFF04..=0xFF07, closure!(clone self.timer, |addr| timer.borrow().read_byte(addr)));
        bus.map_range_write(0xFF04..=0xFF07, closure!(clone self.timer, |addr, data| timer.borrow().write_byte(addr, data)));

        // FF0F - Interrupt Enable
        bus.map_address_read(0xFF0F, closure!(clone self.cpu, |addr| cpu.borrow().read_byte(addr)));
        bus.map_address_write(0xFF0F, closure!(clone self.cpu, |addr, data| cpu.borrow().write_byte(addr, data)));

        // FF10-FF3F - APU 
        bus.map_range_read(0xFF10..=0xFF3F, closure!(clone self.apu, |addr| apu.borrow().read_byte(addr)));
        bus.map_range_write(0xFF10..=0xFF3F, closure!(clone self.apu, |addr, data| apu.borrow().write_byte(addr, data)));

        // FF40-FF4B - PPU Registers
        bus.map_range_read(0xFF40..=0xFF4B, closure!(clone self.ppu, |addr| ppu.borrow().read_byte(addr)));
        bus.map_range_write(0xFF40..=0xFF4B, closure!(clone self.ppu, |addr, data| ppu.borrow().write_byte(addr, data)));
        ...
        match self.model {
            GameBoyModel::GBC => {
                bus.map_range_write(0xFF51..=0xFF55, closure!(clone self.ppu, |addr, data| {
                    ppu.borrow().write_byte(addr, data);
                }));
        ...
        if let GameBoyModel::GBC = self.model {
            bus.map_range_write(0xFF68..=0xFF6B, closure!(clone self.ppu, |addr, data| ppu.borrow().write_byte(addr, data)));
        ...
        bus.map_range_write(0xFF80..=0xFFFE, closure!(clone self.hram, |addr, data| hram.borrow().write_byte(addr, data)));

        // FFFF Interrupt Register
        bus.map_address_read(0xFFFF, closure!(clone self.cpu, |addr| cpu.borrow().read_byte(addr)));
        bus.map_address_write(0xFFFF, closure!(clone self.cpu, |addr, data| cpu.borrow().write_byte(addr, data)));
    ...
    pub fn get_audio_buffer(&self) -> Vec<i16> {
        self.apu.borrow_mut().consume_audio_samples()
    }
    ...
    fn tick(&self) {
        let cpu_cycles = self.cpu.borrow().tick(&self.bus.borrow());
        let clocks = cpu_cycles * 4;

        for _ in 0..clocks {
            self.timer.borrow().tick();
            self.ppu.borrow().tick();
            self.apu.borrow().tick();
        }
    }
